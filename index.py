...
def maybe_finalize_challenge(db: Session, challenge: Challenge):
...
def compute_leaderboard(db: Session, challenge: Challenge):
    challenge = maybe_finalize_challenge(db, challenge)
    ...
    for member in members:
        ...
        today_total = sum(x.value for x in entries if x.entry_date == today_key)
        rows.append({
...
def challenge_summary(db: Session, challenge: Challenge, user_id: str | None = None):
    ...
    winner = leaderboard[0] if challenge.status == 'completed' and leaderboard else None
    return {
        'mode': challenge.mode,
        'type': challenge.type,
...
def auth_telegram(payload: TelegramAuthIn, db: Session = Depends(get_db)):
    ...
    db.commit()
    db.refresh(user)
    return {'user': {'id': user.id, 'telegram_user_id': user.telegram_user_id, 'username': user.username, 'display_name': user.display_name, 'language': user.language, 'timezone_name': user.timezone_name}}
...
def create_challenge(payload: ChallengeCreateIn, db: Session = Depends(get_db)):
    ...
    db.add(challenge)
    db.flush()
    db.add(ChallengeMember(id=uid('mem'), challenge_id=challenge.id, user_id=payload.creator_user_id, role='owner', status='active', sync_status='manual'))
    db.commit()
    db.refresh(challenge)
    return {'challenge': {'id': challenge.id, 'title': challenge.title, 'type': challenge.type, 'mode': challenge.mode, 'trust_mode': challenge.trust_mode, 'duration_days': 7, 'max_members': challenge.max_members, 'status': challenge.status, 'invite_code': challenge.invite_code, 'goal_target': challenge_goal_target(challenge) if challenge.mode == 'goal' else None, 'start_at': challenge.start_at.isoformat(), 'end_at': challenge.end_at.isoformat()}, 'invite_link': f'https://t.me/weekupbot?startapp={challenge.invite_code}'}


@app.get('/api/challenges/{challenge_id}')
def get_challenge(challenge_id: str, user_id: str | None = None, db: Session = Depends(get_db)):
    challenge = maybe_finalize_challenge(db, get_challenge_or_404(db, challenge_id))
    summary = challenge_summary(db, challenge, user_id)
    return {'challenge': {'id': challenge.id, 'creator_user_id': challenge.creator_user_id, 'title': challenge.title, 'type': challenge.type, 'mode': challenge.mode, 'trust_mode': challenge.trust_mode, 'duration_days': 7, 'max_members': challenge.max_members, 'status': challenge.status, 'invite_code': challenge.invite_code, 'goal_target': challenge_goal_target(challenge) if challenge.mode == 'goal' else None, 'start_at': challenge.start_at.isoformat(), 'end_at': challenge.end_at.isoformat()}, 'summary': summary, 'leaderboard': summary['leaderboard']}


@app.get('/api/invites/{code}')
def get_invite(code: str, db: Session = Depends(get_db)):
    challenge = maybe_finalize_challenge(db, db.query(Challenge).filter(Challenge.invite_code == code).first())
    if not challenge:
        raise HTTPException(status_code=404, detail='Invite not found')
    members = db.query(ChallengeMember).filter(ChallengeMember.challenge_id == challenge.id, ChallengeMember.status == 'active').count()
    return {'invite_code': challenge.invite_code, 'challenge_id': challenge.id, 'title': challenge.title, 'mode': challenge.mode, 'type': challenge.type, 'status': challenge.status, 'member_count': members, 'max_members': challenge.max_members}
...
def accept_invite(code: str, payload: JoinInviteIn, db: Session = Depends(get_db)):
    ...
    if existing:
        return {'membership': {'id': existing.id, 'challenge_id': existing.challenge_id, 'user_id': existing.user_id, 'role': existing.role, 'status': existing.status, 'sync_status': existing.sync_status}}
    member = ChallengeMember(id=uid('mem'), challenge_id=challenge.id, user_id=payload.user_id, role='member', status='active', sync_status='manual')
    db.add(member)
    db.commit()
    db.refresh(member)
    return {'membership': {'id': member.id, 'challenge_id': member.challenge_id, 'user_id': member.user_id, 'role': member.role, 'status': member.status, 'sync_status': member.sync_status}}
...
def add_manual_entry(challenge_id: str, payload: ManualEntryIn, db: Session = Depends(get_db)):
    ...
    existing = db.query(Entry).filter(Entry.challenge_id == challenge.id, Entry.user_id == payload.user_id, Entry.entry_date == payload.entry_date, Entry.source_type == 'manual').first()
    if existing:
        existing.value += payload.value
        existing.verified = False
    else:
        db.add(Entry(id=uid('ent'), challenge_id=challenge.id, user_id=payload.user_id, entry_date=payload.entry_date, value=payload.value, source_type='manual', verified=False, raw_payload_hash=None))
    db.commit()
    summary = challenge_summary(db, challenge, payload.user_id)
    return {'status': 'ok', 'message': 'Manual entry stored', 'summary': summary, 'leaderboard': summary['leaderboard']}
...
def get_leaderboard(challenge_id: str, user_id: str | None = None, db: Session = Depends(get_db)):
    challenge = maybe_finalize_challenge(db, get_challenge_or_404(db, challenge_id))
    summary = challenge_summary(db, challenge, user_id)
    return {'challenge_id': challenge.id, 'mode': challenge.mode, 'status': challenge.status, 'summary': summary, 'leaderboard': summary['leaderboard']}
...
def finalize_challenge(challenge_id: str, payload: RematchIn, db: Session = Depends(get_db)):
    ...
    return {'status': 'completed', 'challenge_id': challenge.id, 'winner': winner, 'summary': summary, 'events': ['leaderboard_snapshot_persisted', 'notifications_queued', 'rematch_draft_prepared']}
