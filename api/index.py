...
def _summary(db: Session, challenge: Challenge, user_id: str | None = None):
    ...
    you = next((r for r in lb if r["user_id"] == user_id), None) if user_id else None
...
def get_invite(code: str, db: Session = Depends(get_db)):
    ...
    count = db.query(ChallengeMember).filter(ChallengeMember.challenge_id == ch.id, ChallengeMember.status == "active").count()
    return {
        "invite_code": ch.invite_code,
        "challenge_id": ch.id,
        "title": ch.title,
...
def get_leaderboard(challenge_id: str, user_id: str | None = None, db: Session = Depends(get_db)):
    ch = _challenge_or_404(db, challenge_id)
    summary = _summary(db, ch, user_id)
    return {"challenge_id": ch.id, "summary": summary, "leaderboard": summary["leaderboard"]}
...
def finalize_challenge(challenge_id: str, payload: RematchIn, db: Session = Depends(get_db)):
    ...
    return {"status": "completed", "challenge_id": ch.id, "winner": winner, "summary": summary}
